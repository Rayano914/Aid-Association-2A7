...
int EXCEL::export2Excel()
{
    ...
    QSqlDatabase db = QSqlDatabase::addDatabase("QODBC", "caisse");
    if(!db.isValid())
    {
    ...
    // set the dsn string
    QString dsn = QString("DRIVER={Microsoft Excel Driver (*.xls, *.xlsx, *.xlsm, *.xlsb)};FIL=Excel 12.0;DSN='';FIRSTROWHASNAMES=1;READONLY=FALSE;CREATE_DB=\"%1\";DBQ=%2").
    ...
    if(!db.open())
    {
        ...
        QSqlDatabase::removeDatabase("caisse");
        return -3;
    }
